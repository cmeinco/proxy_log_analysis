...
# 1157689324.281   1465 10.105.21.199 TCP_REFRESH_HIT/304 215 GET http://www.goonernews.com/shop/arsenal_shop_ad.jpg badeyek DIRECT/207.58.145.61 -
# 1157689325.734   1452 10.105.21.199 TCP_REFRESH_HIT/304 214 GET http://www.goonernews.com/flags/FUS.gif badeyek DIRECT/207.58.145.61 -
# 1157689325          2 10.105.21.199 TCP_HIT/200 1353 GET http://www.goonernews.com/flags/FGB.gif badeyek NONE/- image/gif
...
#
    ...
    fullmatch = re.compile(r'' +
        ...
        '^(?P<datetime>[\d\.]+)\s+(?P<bytes_out>\d+)\s+(?P<src_ip>[\d\.]+)\s+(?P<connection_result>[^\/]+)/(?P<http_response_code>\d+)\s+(?P<bytes_in>[\-\d]+)\s+(?P<request_method>[^ ]+)\s+(?P<full_uri>[^ ]+)\s+(?P<user>[^ ]+)\s+(?P<somelastthing>[^ ]+)\s+(?P<response_mime_type>.*)$'
        )

    #open file for output stream
    # Open a file
    fo = open("outputSQ.csv", "w")
